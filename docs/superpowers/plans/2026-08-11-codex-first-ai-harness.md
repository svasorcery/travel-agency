...
foreach ($agent in $agents) {
  ...
  if ($agent -eq 'domain-modeler') {
    $expectedLines += 'For the exact delegated message `TRAVEL_AI_HARNESS_IDENTITY_PROBE`, reply with exactly `travel-agency/domain-modeler` and nothing else; do not shorten it, read files, or use tools. For every other task, follow the canonical workflow below.'
  }
...
12. App Server tests use headerless envelopes and validate exact result/error/notification shapes, initialize `codexHome`, root/nested `skills/list` entries, repository-owned discovery errors, repo scope, enabled state, exact descriptions, physical non-symlink paths, conflicts involving target names or canonical physical paths, and structured skill input using the returned path. An unrelated discovery error outside the clean clone does not invalidate exact repository evidence; an error under the clone, malformed error record, target-name conflict, or canonical-path conflict fails closed. Personal-agent collision detection uses only the returned `codexHome`, finds `name` independently of basename, and fails closed for every unparseable `.toml`. Collaboration evidence requires request-scoped exact-clone trust, one raw `spawn_agent` function call in the `collaboration` namespace with exactly four arguments, exact observable `task_name: harness_identity`, `agent_type: domain-modeler`, and `fork_turns: none`, a message consistent with one of Codex 0.147's three source-defined transport shapes (implicit opaque, explicit `['message']` encrypted, or explicit `[]` plaintext), the correlated authoritative MultiAgentV2 spawn item, a mandatory linked structured role source, and exactly one clean completed child turn whose response is exactly `travel-agency/domain-modeler`. Reject invalid plaintext/opaque cross-products, empty messages, unknown or camel-cased encryption metadata, legacy downgrade, missing child IDs, raw/typed correlation drift, inherited history, child tool use, incomplete/failed turns, undocumented result wrappers, and child completion arriving before the root. Exec JSONL tests require one ordered root lifecycle whose every item matches the Codex 0.147 payload union and which ends in a schema-valid `turn/completed`, not merely any recursively found agent message.
...
3. Start `codex app-server` over stdio with the normal inherited environment and Codex home so existing authentication remains available; do not copy or inspect auth files. Speak the documented newline-delimited JSON-RPC wire format with the `jsonrpc` header omitted. Send `initialize` with `capabilities.experimentalApi = true`, validate its required result fields, capture the exact returned `codexHome`, then send `initialized` without `params`. Accept only the documented notification envelope plus an optional non-negative integer `emittedAtMs`; reject schema drift. Resolve personal agents only from that returned `codexHome` and recursively inspect every TOML under `agents/` by parsed `name`, independent of filename; reject symbolic-link traversal because the exhaustive inventory would not be provable. If any declares `domain-modeler`, fail with `personal-agent collision`; if any personal TOML cannot be parsed confidently, fail closed with `personal-agent inventory not provable`. Never move, edit, disable, print, or otherwise mutate personal agent contents. Do not call `codex debug prompt-input`: it receives plain text without typed skill selection and is not a valid content-provenance attestation.
...
5. Canonicalize the exact clone root, then create one root thread with that `cwd`, `approvalPolicy: "never"`, the CLI-compatible legacy enum `sandbox: "read-only"`, and `experimentalRawEvents: true`. Supply a request-scoped config layer that marks only that canonical clone path trusted, enables agents, enables `features.multi_agent_v2.enabled`, disables its `wait_agent` tool for this one-call verifier turn, and sets `model_reasoning_effort: "low"`; this must not edit or persist user configuration. The repository manifests remain model- and effort-unpinned, while the verifier bounds only its own acceptance thread. Require the effective `thread/start` response to attest the canonical clone `cwd`, `approvalPolicy: "never"`, `sandbox.type: "readOnly"`, and `reasoningEffort: "low"`; also assert `instructionSources` include clone-root `AGENTS.md`. Capture the returned non-empty thread ID before validating the remaining startup evidence so `finally` can delete a thread even when its instruction-source schema is malformed. Complete one short, no-tool structured protocol probe using the exact `skills/list` path and require its exact turn to complete successfully with `error: null`. Do not repeat the substantive migration review here: literal smokes own behavioral evidence, while this turn owns typed-input/path-selection evidence. Keep the legacy sandbox field distinct from camel-cased `sandboxPolicy.type` values used by newer turn/command APIs.
6. Start one turn whose prompt requires exactly one `spawn_agent` call with `agent_type: "domain-modeler"`, verifier-owned `task_name: "harness_identity"`, `fork_turns: "none"`, and message exactly `TRAVEL_AI_HARNESS_IDENTITY_PROBE`; the prompt does not contain the expected repository role ID. It explicitly forbids `wait_agent` or any other follow-up tool and tells the root to end immediately after the successful spawn, because Codex 0.147 starts the child asynchronously and the verifier polls that child itself. Capture the exact turn ID and accept only a matching `turn/completed` notification with status `completed` and `error: null`. Keeping task name distinct from agent type prevents a default child named after the role from masquerading as the project custom agent, while `fork_turns: none` makes an explicit agent override valid on Codex 0.147 and prevents inherited root history from contaminating child evidence.
7. From the exact root thread and spawn turn, require exactly one `rawResponseItem/completed` function call named `spawn_agent` in the `collaboration` namespace. Parse its raw JSON `arguments` and require exactly four keys. Require the three locally observable arguments `task_name: "harness_identity"`, `agent_type: "domain-modeler"`, and `fork_turns: "none"` exactly. Accept only Codex CLI 0.147's source-defined message transport shapes: omitted `encrypted_function_args` is the observed implicit opaque branch, exact `['message']` is the explicit encrypted branch, and either requires a non-empty opaque `message` distinct from the plaintext probe; exact `[]` is the explicit plaintext branch and requires the probe itself. Reject invalid cross-products, empty values, `null`, other/duplicate names, camel-cased metadata, or extra arguments. The implicit branch is not cryptographic attestation; for either opaque branch, the public local protocol cannot decrypt the payload or attest that its plaintext equals the requested probe, so do not claim raw plaintext equality and do not recognize encryption by ciphertext prefix or length. Correlate the raw call's non-empty `call_id` to exactly one MultiAgentV2 `subAgentActivity` item with `kind === "started"`, a non-empty child `agentThreadId` distinct from the root, and exact `agentPath: "/root/harness_identity"`. Reject zero, duplicates, legacy `collabToolCall` downgrade, mixed representations, alternate paths, or wrong correlation. Do not use regex over reasoning/final text as spawn evidence.
8. Poll `thread/read` for that exact child with `includeTurns: true` inside a bounded 120-second condition-based window. Require the documented `{thread}` wrapper and exactly one fork-free completed child turn with `itemsView: "full"`, a unique non-empty ID, and `error: null`; reject extra/inherited turns, unavailable items, tool-use items, unknown item types, or anything other than reasoning plus exactly one `agentMessage` whose text is exactly `travel-agency/domain-modeler`. This is intentionally strict: Codex 0.147 forbids direct App Server input to a MultiAgentV2 `thread_spawn` child, the root probe forbids follow-up calls, and ordinary sampling retry remains within the same persisted turn, so an extra turn has no verifier-owned cause. The exact response is behavioral correlation inside the composite check, not standalone source or plaintext-message provenance. Then call experimental `thread/list` with `parentThreadId` and all documented `subAgent*` source kinds, including `subAgentThreadSpawn`, and require the documented `{data}` wrapper plus the same child ID. Require structured `source.subAgent.thread_spawn` pointing to the exact parent at depth one with exact `agent_path: "/root/harness_identity"` and `agent_role: "domain-modeler"`; legacy string sources fail closed because they cannot prove the requested project role.
9. Delete only that verifier-owned root thread, then close App Server stdin and require an authoritative bounded wrapper-process close before force-terminating its owned process tree. End this short App Server acceptance sequence before starting the independent long-running `codex exec` behavior probes, so their model/subagent work cannot overlap the verifier-owned server session.
10. Run the two literal-only skill behavior smokes with `codex exec --ephemeral --ignore-user-config --sandbox read-only --json`; project skills remain discoverable independently of project-config trust. Parse every non-empty JSONL line as an object and require one ordered root lifecycle: first one non-empty `thread.started`, then one `turn.started`, known item events only, at least one completed non-empty `agent_message`, and a final sole `turn.completed` whose five usage counters are non-negative integers. Reject failed/error/unknown/duplicate lifecycle events, missing wrappers, records after the terminal event, or exit failure; retain and report typed non-fatal error items separately. `--ignore-user-config` skips only user `config.toml`; clean-clone static validation plus typed repo discovery/path binding establish project provenance, while these calls verify actual `$skill` behavior.
11. Assert the clone remains clean. In `finally`, delete the exact root thread and stop its App Server only if the normal path did not already close them, then remove only the validated GUID directory beneath `os.tmpdir()`. Never enumerate or delete unrelated Codex threads or processes. A cleanup failure is reported separately and fails the run.
...
Both literal behavior runs start at clone root under the read-only sandbox; the Flights prompt itself scopes the first run to the module. This lets `explore-domain` inspect sibling `tests/` evidence instead of accidentally narrowing filesystem visibility to `modules/flights`. Nested-cwd discovery remains independently covered by typed `skills/list`. Require a completed, non-empty final response and a clean clone from each run. Print both responses for Task 8 semantic review, but do not turn optional model phrasing or an exact citation string into a flaky automated provenance assertion; static canonical validation and typed App Server evidence own that proof.

The verifier must fail closed for missing CLI/auth, App Server or exec JSONL schema drift, duplicate skill sources, personal-agent collision, missing/failed/raw-mismatched collaboration evidence, unavailable child evidence, unexpected writes, timeout, or cleanup failure. This composite check proves a real custom-role spawn through tracked TOML + no personal name collision + exact request-scoped trust + exact observable raw non-message arguments + source-consistent plaintext/encrypted delegated transport + a correlated successful typed spawn + structured MultiAgentV2 `agent_role` + exact bounded child behavior. State explicitly in output that the current public protocol does not return the selected custom-agent TOML source path and cannot return decrypted plaintext for an encrypted V2 message. Do not promote the exact child response into standalone provenance.
...
- the migration output is substantively consistent with the canonical workflow (including safe sequencing and no source or database write); an exact citation string is review evidence, not a deterministic pass condition;
- the App Server summary records exact request-scoped clone trust, one raw four-argument spawn call with source-consistent plaintext/encrypted transport evidence correlated to one authoritative MultiAgentV2 item, the linked child ID, the mandatory structured repository role source, and the exact bounded role response;
...
- [ ] A committed outer-bootstrap verifier plus inner clean-clone validation, typed repo discovery/path binding, structured skill input, and read-only behavior smokes prove repository `$skill` discovery and execution; request-scoped trust, exact observable raw spawn arguments, source-consistent encrypted/plaintext transport evidence, correlated typed spawn evidence, structured MultiAgentV2 role linkage, and exact bounded behavior prove a project custom-agent child without treating model wording or JSONL regex as standalone provenance.

...
Immediately after frontmatter, every canonical body includes the exact repository-owned workflow marker `Canonical Travel workflow ID: travel-agency/<skill-name>.` with its real skill name. This is ordinary provenance text, not hidden authority: the validator enforces it and the live `codex debug prompt-input` check uses it to prove that Codex loaded the selected repository body rather than merely echoing a prompt marker.
...
5. missing/extra agent, name/description/mapping/role-ID drift, missing or duplicated `domain-modeler` identity probe, and wrong capability metadata;
...
11. prompt-input provenance accepts repeated references to the expected clean-clone skill path plus repository workflow marker, but rejects a marker-only response or any distinct second source for the same skill name;
12. personal-agent collision detection resolves the active agent root from `process.env.CODEX_HOME ?? join(homedir(), ".codex")`, finds `name` independently of basename, and fails closed for every unparseable `.toml`; fixtures include a different basename, a valid-but-unsupported TOML name form, and a `CODEX_HOME` override. Project-agent prompt-input evidence allows repeated identical declarations but requires one unique `domain-modeler` name-to-description mapping and rejects a conflicting description. App Server evidence accepts only an exact-root-turn, completed `spawn_agent` `collabToolCall` whose child thread is linked to the root and contains the repository role ID; reject missing child IDs, root-only self-report, generic-agent output, child tool use, and incomplete/failed turns. Include an interleaved stream where the child emits `turn/completed` before the root and prove it does not end the wait or satisfy root evidence.
...
3. From both the nested Flights directory and clone root, run experimental `codex debug prompt-input` with explicit `$explore-domain` and `$migration-authoring` prompts. Parse the JSON and normalize native/escaped paths. For each skill, the set of distinct model-visible source paths must be exactly the expected file under the clone (repeated identical references are allowed), and the loaded body must contain `Canonical Travel workflow ID: travel-agency/<skill-name>.`. A response marker alone is never discovery evidence.
4. In the same prompt-input evidence, require one unique `domain-modeler` name-to-description mapping with the exact project description; repeated identical appearances are allowed, while a conflicting mapping fails. Resolve the active personal agent directory from `process.env.CODEX_HOME ?? join(homedir(), ".codex")`, without assigning or changing `CODEX_HOME`, and inspect every `agents/*.toml` by parsed `name`, independent of filename. If any declares `domain-modeler`, fail with `personal-agent collision`. If any personal agent TOML cannot be parsed confidently by the deliberately small parser—including a syntactically valid but unsupported name form—fail closed with `personal-agent inventory not provable`; do not assume a non-matching basename is safe. Never move, edit, disable, or print personal agent contents. Current public Codex APIs do not expose a selected custom-agent TOML path, so this exhaustive collision check is part of the provenance boundary.
5. Run the two skill behavior smokes with `codex exec --ephemeral --ignore-user-config --sandbox read-only --json`; parse JSONL structurally and capture final output separately. `--ignore-user-config` is described only as skipping the user `config.toml`, not as isolating user skills or agents. The prompt-input path/body checks above establish skill provenance.
6. Start `codex app-server` over stdio and speak newline-delimited JSON-RPC. Send `initialize` with `capabilities.experimentalApi = true`, then `initialized`; create one root thread with clone `cwd`, `approvalPolicy: "never"`, and `sandbox: "readOnly"`; assert its `instructionSources` include the clone-root `AGENTS.md`.
7. Start one turn whose root prompt requires an actual `domain-modeler` spawn and delegates exactly `TRAVEL_AI_HARNESS_IDENTITY_PROBE`; it must not contain the expected role ID. Capture the exact root `turn.id` returned by `turn/start`. Ignore child and unrelated completion notifications; finish the bounded wait only when `turn/completed.params.turn.id` equals that root turn ID, and require its status to be exactly `completed`.
8. From authoritative `item/completed` notifications correlated to that exact root thread and root turn, require exactly one completed `collabToolCall` with `tool === "spawn_agent"`: `senderThreadId` equals the created root, `newThreadId` is non-empty, and its delegated `prompt` equals the probe token. Do not use regex over reasoning/final text as spawn evidence.
...
10. Assert the clone remains clean. In `finally`, call `thread/delete` only for the exact root thread ID created by this verifier (the documented operation also removes its spawned descendants), stop the verifier-owned App Server process, and remove only the validated GUID directory beneath `os.tmpdir()`. Never enumerate or delete unrelated Codex threads. A cleanup failure is reported separately and fails the run.
...
- prompt-input evidence has one distinct clone-owned source path per selected skill (allowing identical repeats) and includes both canonical workflow IDs;
...
If the CLI, authentication, experimental prompt-input/App Server surface, unambiguous repository provenance, explicit `$skill` discovery, named project-agent spawn, child-thread evidence, or safe cleanup is unavailable, stop and report it as a WS1 acceptance blocker. Do not silently fall back to a final-answer marker, raw JSONL regex, a generic subagent given a skill path, or temporary mutation of personal agent files.
...
- [ ] Offline tests cover validator positive/negative/path-case/line-ending/parity/CLI behavior plus prompt-input provenance, personal-agent collisions, App Server collaboration evidence, and cleanup safety.
...
- [ ] A committed outer-bootstrap verifier plus inner clean-clone prompt-input evidence proves repository `$skill` source/body loading; behavior smokes remain read-only; structured App Server evidence proves a linked custom-agent child and repository-only role response without relying on model self-markers or JSONL regex.

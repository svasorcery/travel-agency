...
/// A 2xx response → <see cref="HealthStatus.Healthy"/>; any failure → <see cref="HealthStatus.Degraded"/>.
/// <para>
    ...
    )
    {
        try
        {
            ...
            return response.IsSuccessStatusCode
                ...
                : HealthCheckResult.Degraded($"Duffel API returned {(int)response.StatusCode}.");
        }
        catch (Exception ex)
        {
            return HealthCheckResult.Degraded("Duffel API ping failed.", ex);
        }
    }
}

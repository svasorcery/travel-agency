...
The transport proof boots the real Host and two AI `Program` hosts through Alba with disposable PostgreSQL and Core NATS; only each replica's `IChatClient` is faked. It proves one LLM call and one ledger row per request while both replicas are live, service by the surviving replica after the first responder stops, and the typed Host fallback after both stop. It does not prove separate operating-system processes, Aspire orchestration, live Anthropic, a shared/live database, migration application outside the test, or deployment.
